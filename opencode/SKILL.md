...
### Exhaustive mode — parallel fan-out with `Task` subagents + a deterministic clustering CLI

A single prompt runs sequentially: in `quick`/`standard` you fetch comparators one at a time, which is fine at 3–10. At 10+ that serial fetch is the bottleneck. So `exhaustive` mode **fans out one `Task` subagent per candidate** (they run concurrently) and **delegates the clustering math to a real Node script** so peer grouping, our-repo classification, and the gap/tier counts are deterministic — not re-derived by the model each run. You act as the coordinator. Discovery and persistence stay here; parallel profiling and the deterministic stages are split out:

The clustering core lives at `~/.config/opencode/skills/sota-scan/cluster-cli.mjs` (a thin CLI over `lib/cluster.mjs`, the source of truth). It exposes two stdin→stdout subcommands — `cluster` (before synthesis) and `finalize` (after) — that sandwich the model synthesis step.

1. **Discover inline (Step 1).** Run the saturation loop yourself — discovery is iterative (each round's queries depend on the last), so it doesn't parallelize. Produce a **broad candidate pool** `[{repo, url, type}]` — aim for **50+** when the domain supports it, since clustering splits them into peer groups.

2. **Build the Us inventory inline (Step 0)** as `[{id, name, status, file}]`, and a one-object **Us profile** `{primary_domain, methodology, target_user, feature_categories:[…], cluster_label}` (Step 1.5) so we can classify our repo into a cluster. Write the Us profile to `/tmp/sota-scan/us_profile.json`.

3. **Profile every candidate in parallel — one `Task` subagent per repo.** Spawn them concurrently (don't await one before starting the next). Give each subagent ONE repo and have it return a strict JSON **profile** object with these fields:
   `{ repo:"owner/name", url, description, readme_summary, primary_domain, methodology (core approach/architecture), target_user, feature_categories:[3–6 kebab tags], cluster_label (broad methodology FAMILY in kebab-case — pick the most generic family that fits, 1–3 words, by HOW it works; never a per-repo label), type:"canonical|popular|technically advanced|niche-relevant|stale-reference", maturity:{stars, last_commit (ISO date), has_docs, has_tests, has_examples, releases}, why (one line: why it's in the field, by type not stars), capabilities:[{id (reuse rubric ids when supplied), name, evidence:{kind:"code|docs|changelog|registry|blog|secondary", reference:"owner/name → path-or-feature; file path only if confirmed"}, confidence:"high|medium|low"}] }`.
   Each subagent must: pull hard signals with `gh api repos/{repo} --jq '{stars:.stargazers_count, pushed:.pushed_at, archived:.archived}'` (leave `stars` null rather than guess if `gh` is unavailable); read README/docs and, where it matters, code; and prefer evidence `code > docs > changelog > registry > blog > secondary` — never assert a capability from a README marketing claim alone. **The `cluster_label` is the most important field and each subagent picks it blind to the others, so insist on coarse families** (✅ `deep-research-agent`, ❌ `iterative-llm-deep-research`) — bespoke labels fragment the field.
   Collect every returned profile into a JSON array and write `{ profiles:[…], us_profile:<the Us profile>, now:"<today>", domain:"<domain>" }` to `/tmp/sota-scan/cluster_in.json`. (If a saved rubric exists, also tell each subagent to reuse its capability ids.)

4. **Cluster deterministically (no model).** Run:
   ```bash
   mkdir -p /tmp/sota-scan
   node ~/.config/opencode/skills/sota-scan/cluster-cli.mjs cluster \
     < /tmp/sota-scan/cluster_in.json > /tmp/sota-scan/cluster.json
   ```
   `cluster.json` contains `{ clusters, merges, user_cluster, selection, selection_explanation, directProfiles, referenceProfiles, allProfiles }`. Read **only** `user_cluster`, `selection`, `selection_explanation`, `directProfiles`, and `referenceProfiles` for the next step — that keeps our context lean (the direct cluster + a couple of references per other cluster, not all 50+).

5. **Synthesize the cluster-scoped matrix + sectioned gaps** (inline, or via one `Task` subagent). Given `directProfiles` (the rubric/matrix MUST be derived from what these same-cluster peers expect), `referenceProfiles` (OTHER clusters — for optional cross-cluster ideas ONLY, never table-stakes gaps), the Us inventory, and the saved rubric if any, produce a JSON **synthesis** object:
   `{ rubric:{domain, version, capabilities:[{id, tier:"table-stakes|edge", test}]}, matrix:[{id, name, us:"met|partial|missing", sota (which DIRECT-cluster repos show it), tier, reference (owner/repo → file/feature, never empty, never a guessed path), confidence}], coverage:{met, total, pct} (over TABLE-STAKES only; pct=round(met/total*100)), gaps:[{rank, capability, section:"direct-peer|maturity|onboarding|cross-cluster", source_cluster (set when the evidence is from another cluster — then section MUST be "cross-cluster"), kind, tier, effort, gap_confidence, impl_confidence, needs_verification, why, study (owner/repo → file — inspiration, NOT a verified call), step1 (patch-oriented: first file to create/edit AND first function/command/test)}] }`.
   Honor Step 3's rules: keep `gap_confidence` and `impl_confidence` separate; don't prescribe an unverified external API as the settled patch. Write this object to `/tmp/sota-scan/synthesis.json`.

6. **Finalize deterministically (no model).** This enforces the honest gap sectioning and the tier count — any cross-cluster gap is forced optional/strategic and excluded from the tier, regardless of how synthesis labeled it:
   ```bash
   node -e 'const fs=require("fs");process.stdout.write(JSON.stringify({domain:"<domain>",cluster:JSON.parse(fs.readFileSync("/tmp/sota-scan/cluster.json","utf8")),synthesis:JSON.parse(fs.readFileSync("/tmp/sota-scan/synthesis.json","utf8"))}))' \
     | node ~/.config/opencode/skills/sota-scan/cluster-cli.mjs finalize > /tmp/sota-scan/result.json
   ```
   `result.json` is the final benchmark object: `{ domain, clusters, merges, user_cluster, selection, selection_explanation, field, rubric, matrix, coverage, tier, gaps_total, gaps:{direct_peer, maturity, onboarding, cross_cluster} }` — the **tier and `gaps_total` here are authoritative**; don't recompute them yourself.

7. **You persist and render (Step 4 + output shape).** Read `result.json`, write `.sota/rubric.<domain>.json` and `.sota/last-scan.json` into the **scanned repo** (or the read-only fallback), diff against the prior scan, and render the dashboard. All persistence logic lives here in exactly one place, so read-only handling is identical across modes.

If the `Task` tool or `node` is unavailable (e.g. a restricted/headless run), fall back to a sequential `exhaustive` scan inline — do the clustering by hand following Step 1.5 — and **disclose** that the parallel fan-out / deterministic CLI was skipped.
...
The deterministic contract lives in **`lib/cluster.mjs`**, wrapped by **`cluster-cli.mjs`** in this skill's directory. In `exhaustive` mode you run that CLI (see "Exhaustive mode" above) so the grouping is reproducible. In `quick`/`standard` mode you do it inline, by hand, following the same shape:
